    ...
    <div className="grid gap-8 md:grid-cols-2">
      {myProjects.map(project => (
        <div key={project.id} className="border rounded-lg p-4 shadow">
          ...
          <p className="mb-1"><strong>Role: Sole contributor</strong> {project.role}</p>
          <p className="mb-1"><strong>Description:
        ...
        · Designed an investment evaluation module to compute ROI timelines and guide optimal capital deployment.</strong> {project.description}</p>
          <p className="mb-1"><strong>Output:
            ...
            Population and number of stations in that area, distance to nearby stations and their performance, Point of Interest (POI) nearby, land price, charger type price, hardware cost, miscellaneous costs and contingency to calculate: total investment, new site performance and ROI</strong> {project.output}</p>
          <p className="mb-1"><strong>Skills:
          ...
          · K-Neighbors
          · Investment Strategy
            </strong> {project.skills.join(", ")}</p>
          <p className="mb-1"><strong>Deliverables:
            · Interactive web dashboard for finding optimal locations
            · Predictive model for site performance
            · Investment costs
            · Return of investment
            ·</strong> {project.deliverables.join(", ")}</p>
